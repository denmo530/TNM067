    ...
    util::forEachPixel(outputImage, [&](ivec2 outImageCoords) {
        ...
        switch (method) {
            case ImageUpsampler::IntepolationMethod::PiecewiseConstant: {
                // Task 6
                // Update finalColor
                // Find nearest pixel position round if between pixels
                ivec2 nearest_pixel_pos = glm::round(inImageCoords);
                
                finalColor = inPixels[inIndex(nearest_pixel_pos)];
                break;
            }
...
dvec2 ImageUpsampler::convertCoordinate(ivec2 outImageCoords, size2_t inputSize, size2_t outputSize) {
    ...
    dvec2 scale_factor = dvec2(inputSize) / dvec2(outputSize);
